...
class MMGATLayer(nn.Module):
    def __init__(self, in_feat, out_feat, num_rels, bias=None,
                 activation=None,dropout=0.4,residual = 0.1):
    ...
    def forward(self, g):
        ...
        def reduce_func(nodes):
            ...
            for data_index in range(self.num_rels):
                ...
                #
                # fz = fz.unsqueeze(dim=-1)
                # fz = fz.unsqueeze(dim=-1)
                # fz = (fz.expand_as(result))
                # # print(result.shape)
                # result = torch.mul(result ,fz)
                # # print(result.shape)
                # result_img = torch.mul( result_img, fz)

                attention_fin=result.sum(dim=1)



                # print("attention_fin.shape{}".format(attention_fin.shape))
                attention_fin_img = result_img.sum(dim=1)
                # print(fz)
                # attention_fin_img = torch.mul( attention_fin_img,fz)
